...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    for col in summary.columns:
        if col.dtype == 'object' and col.unique >= 25:
    ...
    for col in summary.columns:
        if col.is_numeric and col.min is not None and col.max is not None and col.mean is not None:
            if col.min == 0:  # есть нули
                if col.max == 1:  # бинарный (0-1)
                    if (1 - col.mean) > threshold:
                        has_many_zeros = True
                        break
                elif col.mean < col.max * threshold:  # небинарный
                    has_many_zeros = True
                    break
